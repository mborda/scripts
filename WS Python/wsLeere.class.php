...
class wsLeere {
	...
	public function getLibrosCliente($idLibro=0,$idModelo=0){
		if ($this->token){
			...
			return (json_encode($this->do_call('usuarios','getLibrosCliente', $params)));
		}else{
			return (0);	
		}
	}	
	...
	public function getDetalleLibro($idLibro=0,$idModelo=0){
		if ($idLibro){
			...
			return (json_encode($this->do_call('libros','getDetalleLibro', $params)));
		}else{
			return (0);	
		}
	}
	...
	public function addLibroCliente($idLibro=0,$idModelo=0,$idPedido=0,$test=0){
		if ($idLibro && $idModelo && $idPedido){
			...
			return (json_encode($this->do_call('libros','addLibroCliente', $params)));
		}else{
			return (0);	
		}
	}
	...
	public function getGeneros($IdGenero=0){
		...
		return (json_encode($this->do_call('genero','getGeneros', $params)));
	}
	...
	public function getEditoriales($IdEditorial=0){
		...
		return (json_encode($this->do_call('editorial','getEditoriales', $params)));
	}	
	...
	public function getAutores($IdAutor=0){
		...
		return (json_encode($this->do_call('autor','getAutores', $params)));
	}
	public function getModelos($IdModelo=0){
		...
		return (json_encode($this->do_call('modelo','getModelos', $params)));
	}
	...
	public function getCatalogo($IdAutor=0,$IdGenero=0,$IdEditorial=0){
		...
		return (json_encode($this->do_call('catalogo','getCatalogo', $params)));
	}	
